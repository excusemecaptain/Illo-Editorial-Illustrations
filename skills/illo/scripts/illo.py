...
# Original work: Illo by Trevin Chow (https://github.com/tmchow/illo-skill)
# Copyright (c) 2026 Trevin Chow — see NOTICE and LICENSE for required attribution.
# Agent Zero adaptation: excusemecaptain — added Venice AI backend (venice_generate,
# _venice_record, venice resolution in resolve_backend and cmd_doctor).
# All original logic, structure, and prompts remain unchanged.
"""Illo — editorial illustration engine + setup. OpenRouter / Venice AI, stdlib only.
...
VENICE_DEFAULT_MODEL = "flux-2-max"  # Venice image model
...
BACKENDS = ("codex", "openrouter", "venice")
...
def venice_generate(prompt, api_key, model=None):
    """Venice AI backend. POST to /api/v1/image/generate, returns img_bytes.
    Raises BackendUnavailable on recoverable failure; sys.exit on hard error."""
    use_model = model or VENICE_DEFAULT_MODEL
    payload = {
        "model": use_model,
        "prompt": prompt,
        "width": 1792,
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "illo/1.0 (Agent Zero)",
        },
        data=body,
    )
    try:
        ...
        if e.code in (429, 503):
            raise BackendUnavailable(f"Venice HTTP {e.code}: {detail[:300]}")
        sys.exit(f"Venice API error ({e.code}): {detail[:600]}")
    images = data.get("images") or []
    if not images:
        raise BackendUnavailable(f"Venice returned no images. Response keys: {list(data.keys())}")
    img_b64 = images[0]
    return base64.b64decode(img_b64), {"model": use_model, "id": data.get("id", "")}
...
def _render_one(backend, cfg, prompt, model, refs, want_cost, out_path):
    ...
    if backend == "venice":
        return _venice_record(prompt, model, refs, out_path)
    return _openrouter_record(cfg, prompt, model, refs, want_cost, out_path)


def _venice_record(prompt, model, refs, out_path):
    """Render via Venice AI image API and return manifest record."""
    venice_key = os.environ.get("VENICE_API_KEY")
    if not venice_key:
        sys.exit("No Venice API key. Set VENICE_API_KEY environment variable.")
    # Venice does not support reference images in the same way; we embed
    # character description from the prompt. Model override supported.
    venice_model = model if model and "/" not in model else None  # skip OpenRouter ids
    img, meta = venice_generate(prompt, venice_key, model=venice_model)
    path, w, h = place_image(img, out_path)
    return {"path": str(path), "model": meta["model"], "id": meta.get("id", ""),
            "backend": "venice", "cost": None, "width": w, "height": h}
...
def cmd_doctor(args):
    ...
    if _config_is_stale(cfg):
        ...
        venice_key = os.environ.get("VENICE_API_KEY")
        ready = bool(venice_key)
        lines.append("backend: venice — transport: Venice AI API (VENICE_API_KEY env var)"
                     if ready else
                     "backend: venice — NOT ready: VENICE_API_KEY env var not set")
    else:
        ready = False
        lines.append(f"backend: none ready — install + `codex login`, set an OpenRouter key "
                     f"via `{PROG} init`, or set VENICE_API_KEY env var")
